...
class CrystalBallStatus(StatusEffect):
    def __init__(self, *, duration: int = 4) -> None:
        super().__init__("水晶球", "目标范围改为全图，攻击和技能可选场上任何单位。", duration=duration, tick_scope="owner_turn_start")

    def modify_targeting_range(self, value: int) -> int:
        return 99

    def ignores_direct_unit_target_line(self, battle: Battle, actor: HeroUnit) -> bool:
        return True
