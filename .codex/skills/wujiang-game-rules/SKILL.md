...
13. Validate in three layers when feasible: changed-rule targeted tests first, then the relevant behavior scenarios and AI audit, then the broader suite with `python -m unittest discover -s tests`.
14. After each completed fix batch, report the concrete repair outcome to the user: what was fixed, what audit/test command was run, whether high-signal findings remain, and what remains next.
