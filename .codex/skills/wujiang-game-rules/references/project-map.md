...
- Excel roster layer status: all 370 answered unimplemented Excel heroes are selectable and can enter battle through the registry/random pool, with source stats and pure common capabilities active. Their special questionnaire-driven rules are not considered complete until hand/data-driven rule implementations are added per batch. Completed special Excel heroes currently include `excel_r023` 芙蕾, `excel_r026` 最后的守护者, `excel_r036` 制裁者, `excel_r047` 次郎坊, `excel_r056` 蕾米, `excel_r070` 天蟹, `excel_r071` 雪巨人, `excel_r093` 魔神凯撒, `excel_r094` 杂音, `excel_r113` 诚实天使欧内斯特, `excel_r118` 零崎, `excel_r123` 风魔, `excel_r136` 一刀斩。胧月, `excel_r137` 不死小子, `excel_r139` 丰收之神。索拉, `excel_r158` 魔战士, `excel_r166` E。电击人, `excel_r187` 集团恶魔首领, `excel_r188` 光道的守护者。洁拉, `excel_r326` 蝴蝶刺客。弗伦萨, `excel_r337` 湿地之主。缇娜, `excel_r352` 水忍, and `excel_r379` 妖仙。菊; 347 Excel heroes remain to complete.
...
- Chanter:
  ...
  - Card Transposition is passive: when Chanter is affected by an enemy action, Chanter may swap with one own card. The original action still resolves on the originally declared cells.
