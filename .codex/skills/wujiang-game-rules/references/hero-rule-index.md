...
- Completed special Excel rules:
  ...
  - `excel_r291` / 机甲忍者。赤: `iron_chain_path` costs 0.5 mana after the first free use each turn, damages a front edge-truncated 5-cell line without friendly damage, always pierces, moves beside the nearest unit/terrain anchor on that line when legal, and grants +1 to the next direct damage against that anchor in the current turn. R291-Q001 answer `a` makes the hero's separate 1/2 attack/skill pierce roll backend-random once per declared action, public in logs, and shared by every target of that action.
  - `excel_r356` / 强袭犀牛: on destruction, every other current ally gets non-stacking attack +1 / defense +1 for 3 of that ally's owner-turn ends. A battlefield return tracker waits 3 complete global turn-ring rounds, then restores the same hero body at full hp/mana beside an allied Orc hero when a legal cell exists, preserving fixed turn order and battle-use records; it keeps retrying if blocked.
...
- Current acceptance count: 49 fully audited Excel heroes plus 18 handwritten heroes = 67 / 388 fully accepted. H1.2 adds 6 code-complete audit candidates, so the public audit pool is 73 / 388 (55 Excel + 18 handwritten), 6 await user-run deep audit/manual review, and 315 Excel heroes remain undeveloped.
- Testing ownership from H1.2 onward: the development agent writes the required rule/behavior/audit tests and may perform static checks, but does not run any test that advances turns or simulates matches. `tools/audit_all_heroes.py` is the user-run consolidated entrypoint: it records the complete ordinary test suite (including short-turn tests) before the per-hero deep AI audit. No further hero additions proceed without the user's consolidated results.
