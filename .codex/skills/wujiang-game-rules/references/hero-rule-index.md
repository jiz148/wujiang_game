...
- Latest user override (2026-09-06): finish all developed heroes and AI before consolidated testing. Continue review batches without focused retest gates; older requests to wait below are historical. Design-first baselines, per-hero code/AI comparison, necessary test definitions and static checks remain required.

- Solar follow-up (2026-09-06): use the existing exactly-one-cell Evasion contract below; old general-rule prose describing two cells was stale and has been synchronized. The rejected Judgment fixture declares x=4..9 and proves (3,1) is a legal outside landing while (2,1) is not legal. The allowed boundaries cover all exits with x=3..9 or remove evasion mana with x=4..9. User's 933-test run otherwise shows no current-hero failures; new match 7 avoids lethal Thor counters. Revised fixture awaits focused rerun; no additional hero counted or accepted.

- Compact audit entry (2026-09-06): current-batch runs now produce `review_packet.md/json` with all regression failure headings, all finding categories, every trace location, coverage notes, and Git change indexes. `python detail_test_all_wujiangs.py --current --summarize-only` reorganizes saved evidence without simulations. Old evidence and zero findings never establish current-code acceptance; follow the complete design-first review for first-time heroes and reuse locked baselines for follow-up fixes. See `docs/全武将深度审计使用说明.md` for the exact diff scope and limitations.
...
- Solar Judgment follow-up (2026-09-05): preserving an ultimate applies to a concrete declared area, not a blanket prohibition on using its skill code in that board state. Check actual legal escape destinations against the whole declared footprint. A legal ordinary counter that defeats Solar before the area resolves makes that candidate unusable; existing shields/immunity and enemy range/affected-target constraints must still be respected. The shared counter probe currently feeds Solar's ultimate scoring only. Fresh current-batch evidence confirms Red's public 45/45 declaration rolls and Rhino's seven three-attack turns / four blocked-return wait logs; Solar's counter fix and revised boundary tests await the user's focused rerun.
...
- After an action and all chain/follow-up actions finish, if the active hero was destroyed and the battle has not ended, automatically finish that empty turn and advance the fixed turn ring to the next surviving hero.
...
- Strategy real-room result rule: preconfigured city-attack rooms use team 1 as the attacker and team 2 as the defender. When a bound manual/watch-AI/AI-auto room finishes, the strategy campaign resolves the linked pending battle from that room winner exactly once; duplicate room-state polls are idempotent. `ai_auto` creates an AI-only real grid room and runs it to completion server-side before returning the strategy response, while `quick` remains the sandbox resolver. V0.1 troop writeback counts surviving non-summon/non-clone `strategy_*` soldier units per team and converts them back proportionally against that side's original strategic troop count and capped starting grid-unit count; real strategic hero survivors are tracked separately so heroes do not inflate remaining troop counts. Strategy campaign join codes are host-rotatable; rotating immediately invalidates the old join code and returns the updated campaign payload. V0.1 strategy permissions reserve month advancement, initial-player locking, and join-code rotation for the host; regular members may operate their own faction's city policy, tactic tech, and attacks after the resume gate is satisfied. Resolved strategy battles persist structured `battle_result` data with winner/loser factions and sides, city-captured flag, lost/remaining troops by side, initial/surviving grid-unit counts, strategic hero commit/survival/sleep result, resolution source, and battle log summary; the strategy panel renders these fields directly in battle records instead of parsing report strings, renders a members/invite panel, renders an initial-player online/missing panel from `resume` plus `members`, also renders a compact node-link map from `world.nodes` / `world.cities`, and finished linked battle rooms expose a return-to-campaign path using the synced strategy payload.
- Strategy queued-action rule: current-month strategy plans are persisted outside `WorldState` as campaign actions. Players can queue own-faction city policy changes, tactic-tech unlocks, rebellion handling actions (`appease`, `relief_grain`, `suppress`), sandbox rebel-clearing battles (`rebellion_battle`), and city attacks after the resume gate; queued attacks support `manual`, `watch_ai`, `ai_auto`, and `quick`. Host month advancement applies the queue before monthly settlement, creates queued real-grid rooms when needed, runs queued `ai_auto` rooms to completion server-side, and then clears the old month's queue. Invalidated queued actions log `queued_action_failed` and do not abort the remaining queue. The strategy panel renders rebellion-action controls in owned city cards, shows "计划清剿" for owned cities with rebel forces, and labels queued `rebellion_action` / `rebellion_battle` entries by city and action name.
- Strategy rebellion rule: monthly settlement stores `rebellion_risk:<risk>:<stage>` on cities. Risk 45+ records `rebellion_crisis`; risk 75+ triggers formal rebellion by creating/growing `rebellion_force:<troops>:month:<month>`, damaging city troops/food/money/support, raising `local_autonomy`, and logging `rebellion_uprising`. `suppress` spends city troops and reduces an existing rebel force by twice the troop cost; clearing it logs `rebellion_suppressed`. `rebellion_battle` is the current v0.1 sandbox rebel battle: committed city troops fight `rebellion_force`, victory clears it, failure reduces it while damaging troops/support. City cards render these event states as readable risk/rebel-force labels. Rebel real-grid battles and autonomy/defection branches are future work.
- Strategy monthly AI rule: after queued player actions and before monthly settlement, host month advancement runs deterministic v0.1 AI for factions not controlled by campaign members. Each non-exiled unclaimed faction may adjust one city policy, unlock the first affordable available tactic tech, summon one affordable home-faction strategic hero, set a default defender when it has an active hero and no configured defender, and launch at most one resolved `quick` adjacent city attack when the source city has overwhelming troops. AI policy selection prioritizes city survival risks before ordinary development: food shortage chooses food, high rebellion risk with enough security troops chooses suppression, high rebellion risk without enough troops chooses autonomy, and medium risk with low troops chooses recruitment. AI quick attacks commit currently active strategic heroes up to the faction deployment limit. AI logs `strategy_ai_plan`; it skips player-controlled and exiled factions. Diplomacy, relic release, officer logic, rebellion event chains, AI active pacification/suppression actions, rebel battles, and deeper story-aware AI remain future work.
- Strategy objective/exile rule: public strategy worlds include `strategic_status` plus `exile_action_choices`. V0.1 implements city-count victory checks for `unify_cities` and `eliminate_enemy_factions`, keeps `world_mainline` and `relic_altar` as visible unimplemented placeholders, marks factions with zero controlled cities as exiled, and records exile/victory events once through `memory_tags` during month advancement. Exiled factions may queue `exile_action` plans: `seek_aid` gives food/money/ether, `rally_followers` gives exile troops/food, `build_network` raises target-city support, and `rebuild_base` spends 300 troops plus 120 money to reclaim a weak or supportive target city. Complete ending rewards, deeper exile diplomacy/vassalage, world-mainline objectives, and relic/altar victories are not implemented yet.
- Strategy hero rule: public strategy worlds include `strategic_hero_pool`; each faction also exposes its own `strategic_heroes`. The pool is dynamically derived from `list_heroes()`, so public/selectable handwritten heroes and implemented Excel heroes are included automatically, while internal `strategy_*` soldiers and unimplemented Excel rows are excluded. Campaign seed plus hero code deterministically assigns each strategic hero to a home faction. `summon_strategic_hero` is a monthly queued action that costs `20 + level * 5` ether, writes `strategic_hero_summoned:{code}` to faction memory, and marks that hero `summoned` in public strategy payloads. V0.1 city battles let the attacker choose summoned non-sleeping strategic heroes for immediate or queued attacks up to `strategic_hero_deployment_limit`; this persists on `PendingBattle.attacker_hero_codes`. The default limit is 1, and the `hero_command` tactic tech adds +1, capped by the current strategic max. Players may set one default defender via `strategic_hero_defender:{code}`; `None` defender hero config means use that default if valid, otherwise automatic defender default up to the dynamic limit. The defending faction may also set or clear summoned non-sleeping heroes on a specific pending battle before a real grid room exists; this writes `PendingBattle.defender_hero_codes` and locks once `battle_room_id` is attached. Committed heroes enter manual/watch-AI/AI-auto rosters as extra real hero codes; defeated committed heroes write `strategic_hero_sleeping:{code}:until:{month}`, show `sleeping` in public strategy payloads until the recovery month, then return to `summoned`. Relic binding, hero missions, and officer roles are future work.
...
- Shared required-attack contract: Cat Uncle and Counter Boxer expose the same forced-target status to legality and AI scoring. The affected unit may only move until it declares a basic attack against the required live target; movement scoring closes distance and attack scoring hard-prioritizes that target. A newly applied required-target taunt replaces every older required-target status on that unit so two different taunts cannot create an impossible action lock.
...
- Per-hero AI debug requires 10 simulations for each implemented target hero: target hero + one random implemented teammate vs two random implemented opponents, with varied seeds/teams. The preferred all-roster entrypoint is `python tools/audit_all_heroes.py --matches-per-hero 10 --max-steps 500 --seed N --out reports/all-hero-deep-audit/current --resume`; it writes resumable per-match traces, per-hero action/skill/state metrics, a review queue, and design-intent observation gaps. Before analysis, read the hero source/docs, questionnaire answers, and `docs/武将设计思想.json` / the `武将设计思想` workbook sheet when available. Compare every battle report and structured finding against that baseline, then record and fix general or hero-specific defects before rerunning verification. The audit tool gathers evidence; it does not decide balance or final acceptance by itself.
...
## Internal Strategy Soldier Units

- File: `src/wujiang/heroes/strategy_soldiers.py`.
- These `strategy_*` codes are registered so strategy battle rooms can call `create_battle(...)`, but they are internal city-state strategy units and must stay out of public manual/random hero lists.
- All current v0.1 strategy soldiers have no skills and no traits. They exist to turn strategic troop counts and city-flavored troop ratios into real grid units for `manual` / `watch_ai` battle handoff.
- `strategy_infantry` / 普通步兵: level 1, Swordsman, Earth, Human, attack 2, defense 2, speed 2, range 1, mana 0.
- `strategy_garrison` / 守备兵: level 1, Swordsman, Earth, Human, attack 2, defense 3, speed 1, range 1, mana 0.
- `strategy_archer` / 弓兵: level 1, Archer, Wood, Human, attack 2, defense 1, speed 2, range 4, mana 0.
- `strategy_cavalry` / 骑兵: level 1, Knight, Light, Human, attack 3, defense 2, speed 4, range 1, mana 0.
- `strategy_mountain_soldier` / 山地兵: level 1, Swordsman, Earth, Human, attack 3, defense 2, speed 3, range 1, mana 0.
- `strategy_ether_scout` / 以太侦察兵: level 1, Mage, Lightning, Human, attack 2, defense 1, speed 4, range 3, mana 0.
- `strategy_wall_engineer` / 城墙工兵: level 1, Support, Earth, Human, attack 1, defense 3, speed 2, range 2, mana 0.
...
- Completed special Excel rules:
  ...
  - `excel_r138` / 无名信使: `pierce` and `dragon_breath` keep the common shapes/costs but declare shield pierce. Base mana and initial magic points are both 5. Each declared Dragon Breath or Pierce grants 1 magic point once; `messenger_creation` grants 1 magic point once per turn; `messenger_reincarnation` costs 2 magic points and refills current mana to cap only while current mana is 0. Every skill-damage instance from this hero ignores shields. AI explicitly values Creation and prioritizes Reincarnation at zero mana.
  ...
  - `excel_r327` / 红莲魔。盖泽尔: attack and defense dynamically equal base value plus the number of destroyed non-summon/non-clone heroes. Enemy direct skill damage is cancelled while non-damage skill effects and field/weather damage remain. Basic attacks declare shield pierce and cannot be evaded. Per global `battle.turn_number`, only the first actual damage instance against this hero resolves; cancelled/zero-damage attempts do not consume that allowance. The cached destroyed-hero count is synchronized by the battle engine so strategic battle checkpoints remain serializable.
  - `excel_r120` / 盲人剑士: normal movement may be used repeatedly, each move can travel up to the integer part of current mana, and each traveled cell costs 1 mana. Basic attacks are piercing surrounding-eight-cell area attacks. Only an actual Evasion reaction that moves the declared target out of the attack restores the spent attack count.
  - `excel_r143` / 托尔: `thor_heavy_hammer` costs 1 mana once per turn, damages one adjacent cell, and doubles enemy reaction-skill mana costs for that chain. `thor_rage_impact` costs 1 once per turn, damages the front 4 cells then moves to the legal fifth cell. `thor_destroy_lightning` costs 1 once per turn, damages the front 5 cells, ignores lightning-attribute units entirely, and prevents other effective hits from acting on their next own turn.
  - `excel_r172` / 甲虫装甲骑士: `beetle_spear` is a cooldown-2 front 5-cell current-attack skill. `beetle_full_force` is a start-phase toggle for attack +2, speed +2, defense -3. `beetle_armor_deploy` is a cooldown-2 passive reaction usable only when a hostile action affects the owner; it heals 1/4 and grants defense +3 plus no actions through the end of the owner's next turn.
  - `excel_r224` / 电子镭射龙: common Machine Gun and Ion Shield apply. `electronic_laser` is once per turn and damages a declared front 7-cell line. Direct basic-attack or damaging-skill contexts gain attack power +1 against a target whose current attack is lower; R224-Q001 answer `a` includes fixed direct skill loss but excludes independent field/weather damage. The unit has the rule-name alias `电子龙` without changing its displayed name.
  - `excel_r225` / 电子屏障龙: common Missile, Ion Shield, and Quantum Shield apply. Direct basic-attack or damaging-skill contexts against this unit lose attack power 1 when the source's current defense is lower. Its dynamic surrounding-eight-cell allied aura lowers incoming attack power 1, or 2 for mecha; multiple same-name auras use only the highest reduction. R225-Q001 answer `a` includes fixed direct skill loss but excludes independent field/weather damage. The unit has the rule-name alias `电子龙`.
  - `excel_r264` / 豹头人。飞翼: Fly Leap is free once per turn and resets ordinary movement count after its successful movement. `eagle_eye` has base cost 0.5 with the first use each turn free, selects a range-valid edge-truncated `2*3`/`3*2` area, deals no damage, and applies 3 owner-turn-end rounds of no basic attacks or active skills when defenses do not cancel the effect.
  - `excel_r126` / 太阳神: `solar_flame` is common Remote Dragon Breath under the source name 爆炎. `sphinx_cannon` is cooldown 2, deploys only in a legal surrounding cell, and summons range-7 artillery whose basic attack freely declares a remote edge-truncated `3*3` area rather than centering on a unit; because no side restriction is written, the area hits both sides, and its preview must expose both allied and enemy affected unit IDs. `solar_judgment` is a once-per-battle remote edge-truncated `10*10`, attack-power-6, half-piercing, magic-immunity-ignoring area that hits both sides. Both artillery fire and Judgment resolve unit damage first, then independently destroy terrain in the declared cells; an individual shield/evasion/immunity cancellation does not protect terrain, while whole-action cancellation prevents both. R126-Q001 defines terrain as both non-unit terrain (including board wall cells) and units explicitly treated as terrain; ordinary weather/fields are not terrain unless explicitly marked. The placement, free-area, and damage-then-terrain interpretations were design-inferred and locked on 2026-09-01 from the source/common rules and Solar God's artillery role, not from implementation. Judgment AI must discount enemies with a currently legal reaction that previews movement outside the locked area, and must hold the once-per-battle skill when projected allied defeats are equal to or greater than reliable enemy defeats.
  - `excel_r142` / 猫叔: `cat_taunt_roar` is once per turn and deals current-attack piercing skill damage to every allied or enemy unit in the surrounding `7*7`; each surviving hit unit may only move on its next own turn until declaring a basic attack against Cat Uncle. A forced allied target is exposed as a legal basic-attack target by the general required-target contract. Any allied or enemy attack/skill affecting Cat Uncle costs its source 1 mana once per declared action, pulls it along a legal straight line to the closest reachable position toward Cat, and locks ordinary movement plus active skills for the current turn without cancelling the locked original action. Direct area damage also triggers the reaction even when no single-target validation ran. Deduplication uses the enclosing queued action's resolution token, not effect/action-name strings, so damage and follow-up effects from one declaration share one reaction while genuinely nested declared actions remain distinct.
  - `excel_r198` / 三轮车人: every completed basic-attack declaration adds one normal-move action for the current turn. `summon_bicycle` is cooldown 2, has no same-name field cap, and creates a Bicycle in a legal surrounding cell. Each Bicycle independently resets ordinary movement after a basic attack and may use `summon_unicycle` once per turn; each Unicycle also resets ordinary movement after a basic attack. Both enter unable to act and are ordinary nested summons: direct-summoner removal cascades only through its own branch.
  - `excel_r206` / 反击拳手: `ring_taunt` is once per turn and deals current-attack piercing skill damage to every allied or enemy unit in the surrounding eight cells. Each surviving hit unit with a basic-attack resource may only move on its next own turn until declaring a basic attack against Counter Boxer; allied forced targets are legal. The hero has ordinary Block/Counter plus optional `boxer_block_counter`, which adds defense +1 only for the current chain's next damage and immediately makes a basic-attack-style counter when the hostile speed-1 source is in range. Damaging counters trigger Attack Lifesteal; Pierce does not.
  - `excel_r291` / 机甲忍者。赤: `iron_chain_path` costs 0.5 mana after the first free use each turn, has no written per-turn cap, damages a front edge-truncated 5-cell line without friendly damage, and always pierces. The mandatory anchor is fixed before damage by scanning the declared line near-to-far: stable units of either side, terrain units, board walls, then explicitly terrain-marked fields. A legal move must stay within 5 cells on one straight path and end genuinely beside the saved anchor footprint; failure skips only movement. Only a non-terrain unit anchor grants +1 to Red's next direct damage against that unit in the current turn, even if the anchor dies during the line hit. R291-Q001 answer `a` makes the separate 1/2 attack/skill pierce roll backend-random once per declared action, public in logs, stored only in that action payload, and shared by every target of that action. The roll uses the public-event path so a stealthed target cannot suppress it. AI must score the forced anchor, actual landing exposure, and follow-up opportunity rather than line damage alone.
  - `excel_r356` / 强袭犀牛: on every true destruction, every other current ally gets non-stacking attack +1 / defense +1 for 3 of that ally's owner-turn ends, and the same life creates only one return tracker. A tracker waits 3 complete global fixed-turn-slot rings, then restores the same hero body at full hp/mana beside the first eligible allied Orc hero in fixed turn-slot order and the first stable legal surrounding cell; it keeps retrying if no Orc/body cell is available and logs that due-but-blocked wait. Return clears previous-life temporary states, preserves indefinite numeric modifiers plus battle-use/cooldown records and the original turn slot, grants no immediate action, and re-enables the next life to trigger Legacy and Return again. General AI regenerates candidates after each basic attack so Rhino can use its three attacks sequentially. Its audit action code is `shensu`, and a merely available Dragon Breath with no effective damage candidate is not an AI miss.
...
- Implemented-hero inspection/modification count: 6 / 73 developed/public heroes have completed the new design-first workflow (locked design intent, battle implementation inspection, hero-specific/general AI inspection, test inspection, and necessary corrections); 67 / 73 remain to be inspected and modified. Focused retest is a separate secondary state: Cat Uncle, Tricycle Man, and Counterattack Boxer are accepted after rerun, while Solar God, Mecha Ninja Red, and Assault Rhino await the user's focused rerun. The 315 undeveloped Excel heroes are tracked separately and must not be mixed into this denominator. Overall 388-roster totals are reported only when explicitly requested.
- Testing ownership from H1.2 onward: the development agent writes the required rule/behavior/audit tests and may perform static checks, but does not run any test that advances turns or simulates matches. `python detail_test_all_wujiangs.py --current` records the ordinary suite and audits only `current_hero_codes`; the flagless command is reserved for phase-level all-public audits. An audit must distinguish legal availability from effective candidates, treat only declined effective candidates as high-priority misses, and attribute tracked logs/actions by actual source/target/lifecycle relation instead of name alone. No further hero additions proceed without the user's focused result and manual design-trace acceptance.
- Stable short command for a current one-to-three-hero review batch: run `python detail_test_all_wujiangs.py --current` from the repository root. It reads explicit `current_hero_codes` from `docs/武将符合性审查.json`, keeps the complete ordinary regression, and limits deep simulations to that batch under a code-keyed `reports/current-hero-deep-audit/<batch>` directory. The flagless command remains the phase-level all-public-hero audit.
- Mandatory per-hero review order: read only the source hero text, questionnaire answers, durable clarifications, and common rules; independently infer the hero's intended role, player decisions, cadence, synergy, strengths/weaknesses/counterplay, AI priorities/prohibitions, and boundary behavior; record and lock that understanding in the workbook plus `docs/武将设计思想.json`; only then inspect battle code, then AI code, then tests. Compare each layer against the locked design baseline, classify mismatches, and only afterward modify implementation. Current code/tests must never be used to invent or backfill the intended design, and a skill merely appearing once is not proof of design conformity.
- Ambiguity and batching rule from 2026-09-01: do not pause for user answers to source ambiguities. Infer the most coherent interpretation from source text, durable/common rules, role, cadence, counterplay, and skill synergy; record the decision, rejected alternatives, evidence, confidence, and gameplay consequences before code inspection. Review three heroes per batch by default: lock all three independent source-only baselines first, then inspect and modify each hero at full depth.
...
- R01 review 2026-09-08: locked baseline plus battle/AI/test-code inspection completed; awaiting consolidated testing. Curse prepays fixed 0.5 HP even if blocked; Experiment increases mana capacity and current mana by 2. Mana Pull frontend/AI use backend `destinations_by_target`; only the pulled unit's original footprint is affected for chain discovery. AI prices life/death costs and retains immediate winning Experiment opportunities.
...
- R01 review 2026-09-08: cooldown/whole-round values below supersede obsolete half-round conversion in old prose. Blackout lasts two own rounds; shared cooldown only ticks own ends. Banish skips the next complete own action, then returns at the following own start. AI prices both coin outcomes and unused attacks, values glove reset with resources/target, and prioritizes darkness's empowered attack. Dynamic tests deferred.
...
- R01 review 2026-09-08: cross stays on declaration cells after caster displacement; full footprints and allies count, caster never counts as a damage/AI target. Ember ticks only the affected hero's action bundle, not all teammates; crossing inspects footprint cells. Judgment compares current attack+defense+speed+normal range+mana, exempts all in-play minimum ties, locks remaining cells, resolves attack 6, and shield cancels damage plus no-heal. No-heal lasts three target-own rounds. AI accounts for new ember cells, lost attack value, real friendly fire, magic-immunity bypass and cannot-evade. Dynamic tests deferred.
...
- R02 reviewed 2026-09-09: Precision applies only to basic attacks/counters, never Machine Gun; only slow pierces. AI compares actual attacks before/after Headshot without drawing proc randomness. Machine Gun excludes allies in payload/chain/AI; Backstep uses shared path legality, exactly two straight pass-through cells. Static only; final consolidated tests pending.
...
- R02 reviewed 2026-09-09: Heal reverses to fixed 1/4 HP loss for dark/spirit/demon, with AI scoring actual outcomes. Defend rejects same-caster stacking, Chant feeds useful mana-point recipients. Any movement cancels stationary recovery. Holy Light follows caster 11×11, enemy normal-move endpoint takes attack-4 formula, allies gain defense only at their own ends; expires at caster's third own end including casting turn. Static only; final consolidated tests pending.
...
- Skills:
  ...
  - `heal` / `HealSkill`: pay 1 mana; once per turn; ally/self in range heals 1/4 HP; dark attribute or spirit/demon instead loses fixed 1/4 HP.
  - `protection` / `PassiveProtectionSkill`: passive chain speed 2; pay 1 mana; only self; gain 2 shields until chain end.
  - `baptism` / `BaptismSkill`: pay 2 mana; human ally gains 1-own-round magic immunity from skill damage/effects, not fields.
  - `great_holy_light` / `GreatHolyLightSkill`: once per battle; 2.5-round moving 11×11 field until caster's third own end (cast end counts); enemy normal-move endpoint in field takes attack 4; allied unit's own end grants non-stacking defense +1 until its next own start.
...
- R02 reviewed 2026-09-09: source-only baseline resolves Earth Walker via common Split as three clones; backend/AI/frontend share three-cell selection. Burn/Blizzard damage is ordinary, non-damage control pierces one shield even through damage-only negation; magic immunity still applies. Remote areas lock declared cells. Water Wave cooldown is four own rounds and expiry clamps mana. Plant Growth checks full starting footprints, affects friends/flying and excludes skill/forced movement. AI compares new-control value, friendly terrain harm, and available attacks before ending the body turn. Thunder God reset remains enemy direct damage only. Static only; final tests pending.
...
- Skills:
  ...
  - `earth_walker` / `EarthWalkerSkill`: no mana; once per turn; choose 3 non-overlapping clone positions in range; caster cannot continue acting; clones can move this turn but cannot attack/use skills; caster randomly swaps with one new clone; all expire before ElementHunter's next own turn.
